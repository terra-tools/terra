...
pub struct ColorPalette {
    ...
    // terra patch: selection colors (Ghostty-style opaque overlay instead of
    // an fg/bg swap).
    pub selection_background: String,
    pub selection_foreground: Option<String>,
    // terra patch: cursor beam color.
...
impl Default for ColorPalette {
    fn default() -> Self {
        Self {
            ...
            selection_background: String::from("#3f638b"),
            selection_foreground: Some(String::from("#ffffff")),
            cursor_color: String::from("#98989d"),
        }
    }
}
...
impl TerminalTheme {
    ...
    /// terra patch: background painted behind selected cells.
    pub fn selection_background(&self) -> Color32 {
        let color = &self.palette.selection_background;
        hex_to_color(color)
            .unwrap_or_else(|_| panic!("invalid color {}", color))
    }

    /// terra patch: foreground forced on selected cells, if the theme sets
    /// one. `None` means "keep the cell's own foreground".
    pub fn selection_foreground(&self) -> Option<Color32> {
        self.palette.selection_foreground.as_ref().map(|color| {
            hex_to_color(color)
                .unwrap_or_else(|_| panic!("invalid color {}", color))
        })
    }
